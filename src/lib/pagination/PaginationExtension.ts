...
function createSpacerWidget(height: number, pos: number): HTMLElement {
  const widget = document.createElement('div');
  widget.className = 'page-break-spacer';
  widget.style.height = `${height}px`;
  widget.style.pointerEvents = 'none';
  // Lets usePagination.ts find this exact spacer by the break's document position and
  // read its real rendered rect, instead of resolving that position back through
  // coordsAtPos - which is ambiguous right here, since this widget sits at that same
  // position (see usePagination.ts for the full explanation).
  widget.dataset.breakPos = String(pos);
...
function buildDecorations(
  ...
  const decorations = breaks.flatMap((brk) => {
    ...
    const widget = createSpacerWidget(spacerHeight, brk.pos);
